...
__global__ void cal_entropy(int *A, float *B, int rows, int cols){
    ...
    if(idx < rows*cols){
        ...
        for(int i = 0; i < 16; i++){
            if(digit[i] != 0){
                result += (((float)digit[i])/count)*(tex1Dfetch(tex, digit[i]-1)-tex1Dfetch(tex, count-1));
            }
        }
        // 读入到输出矩阵
        B[idx] = -result;
...
{   
    ...
    float my_log[25] = {
        ...
        3.178054};
    float* Add = NULL;
    cudaMalloc((void**)&Add, 25*sizeof(float));
    cudaMemcpy(Add, my_log, 25*sizeof(float), cudaMemcpyHostToDevice);
    // 绑定
    cudaBindTexture(0, tex, Add,25*sizeof(float));
    //GPU计时
